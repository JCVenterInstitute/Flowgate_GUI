    ...
    def fcytMetadataParse(experiment, fileListMap, headers) {
        fileListMap.each { dataRow ->
            if (dataRow['Category'] && dataRow['Key']) {
                ExperimentMetadataCategory category = ExperimentMetadataCategory.findOrSaveByExperimentAndMdCategoryAndDispOnFilterAndVisibleAndIsMiFlow(experiment, dataRow["Category"], false, true, true)
                ExperimentMetadata metaDat = ExperimentMetadata.findOrSaveByExperimentAndMdCategoryAndMdKeyAndVisibleAndDispOnFilter(experiment, category, dataRow['Key'], true, false)
                if (dataRow['Value']) {
