...
function create_node() {
  console.log('create node, node=root?', create_node_parent);
...
function rename_node() {
  // console.log('rename node, node=root?', node_to_rename.parent===[], node_to_rename.depth);
  if (node_to_rename && rename_node_modal_active && typeof node_to_rename.parent !== 'undefined') {
    ...
    node_to_rename.yM = yM;
    node_to_rename.pop = pop;
    rename_node_modal_active = false;
  }
  close_modal();
  // if(node_to_rename.parent!==[]){
    outer_update(node_to_rename);
  // }
}

function saveBtnClicked(treeData){
  console.log('save btn clicked!', treeData);
}
outer_update = null;
...
function draw_tree(error, treeData) {
  ...
  var saveBtn = window.parent.document.getElementById('aux21SaveBtn');
  if(saveBtn){
    saveBtn.onclick = saveBtnClicked(treeData);
  ...
  var menu = [
    {
      ...
      action: function(elm, d, i) {
        ...
        $("#RenameNodeXM").val(d.xM);
        $("#RenameNodeYM").val(d.yM);
        $("#RenameNodePop").val(d.pop);
        rename_node_modal_active = true;
        node_to_rename = d;
        $("#RenameNodeName").focus();
        $('#RenameNodeModal').foundation('reveal', 'open');
      }//,
      // disabled: function(d){ return d.parent !== 0; }
      // disabled: function(d){
      //   console.log('menu item ',d, d.depth);
      //   return d.depth !== 0;
      // }
        // console.log('menu item ',d);
  ...
  function delete_node(node) {
    // console.log('delete node, node=root?', node, node===root);
    visit(treeData, function(d) {
        if (d.children) {
  ...
  function update(source) {
    ...
    var newHeight = d3.max(levelHeight) * 10; // 25 pixels per line // link height
