...
<div class="row" style="padding:0;position: absolute;height:102%;border-right: #000 solid 1px;max-width:300px">
    ...
    <ul>
        <g:each var="project" in="${projectList}" status="p">
            <li class="${session?.searchLst?.find {it == project?.id} != null ? 'findSel' : ''} folder" style="cursor: pointer" onclick="projectClick(${project?.id})"><i class="fa fa-${session?.projectOpenId?.toLong() == project?.id ? 'folder-open' : 'folder'}-o" ></i>&nbsp;&nbsp;${project?.title}</li>
                ...
                <ul>
                    <g:each var="experiment" in="${experimentList}" >
                        <li >
                            ...
                            <span id="experimentFolder" style="cursor: pointer" onclick="window.location.href='/flowgate/experiment/index?pId=${project?.id}&eId=${experiment?.id}'" ><i class="fa fa-${session?.experimentOpenId?.toLong() == experiment?.id ? 'folder-open' : 'folder'}-o" ></i>
                                &nbsp;&nbsp;${experiment?.title}
