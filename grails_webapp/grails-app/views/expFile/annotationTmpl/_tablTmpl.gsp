    ...
    <g:each in="${experiment.expMetadatas.findAll { it.mdCategory == category }.sort { it.dispOrder }}" var="eMeta">
      <td class="text-center">
        <div id="tblCell_${expFile.id.toString()}_${eMeta.id.toString()}">
          ...
          <g:render template="annotationTmpl/tablCellTmpl"
                    model="[eMeta: eMeta, expFile: expFile, checked: expFile.metaDatas.find { it.mdKey == eMeta.mdKey }?.mdVal == session[sessMetaStr]]"/>
        </div>
      </td>
    </g:each>
  %{--</g:each>--}%
