    ...
    <g:if test="${experiment?.id == session?.experimentOpenId?.toLong()}">
        <h1 class="text-center" style="padding-top: 30px">
            <g:if test="${experiment?.id == session?.experimentEditModeId?.toLong()}">
                <input class="form-control" name="title" value="${experiment?.title}" onchange="experimentTitleInputBlur(${experiment?.id}, this.value)" >
            </g:if>
            <g:else>
        ...
        <div class="jumbotron">
            <g:if test="${experiment?.id == session.experimentEditModeId?.toLong()}">
                <textarea id="experimentDescription" class="form-control" rows="10" cols="130" onchange="experimentDescriptionInputBlur(${experiment?.id}, this.value)" onfocus="this.select()" >${experiment?.description}</textarea>
            </g:if>
            <g:else>
        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <sec:ifAnyGranted roles="ROLE_SuperAdmin,ROLE_Administrator,ROLE_Admin,ROLE_ExperimentEdit">
                ...
                <sec:ifNotGranted roles="ROLE_SuperAdmin,ROLE_Administrator,ROLE_Admin,ROLE_ExperimentEdit">
